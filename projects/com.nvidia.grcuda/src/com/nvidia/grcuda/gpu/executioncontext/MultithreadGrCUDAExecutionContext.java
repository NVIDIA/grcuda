...
public class MultithreadGrCUDAExecutionContext extends AbstractGrCUDAExecutionContext {
        ...
        if (vertex.getComputation().canUseStream() && threadManager != null) {
            ...
            if (vertex.isExecutable()) {
                ...
                CompletableFuture<Object> futureResult = CompletableFuture.supplyAsync(
                        () -> new ComputationThread(vertex).call(), threadManager.getThreadPool())
                        .exceptionally(ex -> {
                            System.out.println("Exception encountered in computation thread of "
                                    + vertex.getComputation() + ": " + ex.getMessage());
                ...
                CompletableFuture<Object> futureResult = parentResult.thenCompose((res) -> CompletableFuture.supplyAsync(() -> new ComputationThread(vertex).call(), threadManager.getThreadPool())
                        .exceptionally(ex -> {
                            System.out.println("Exception encountered in computation thread of "
                                    + vertex.getComputation() + ": " + ex.getMessage());
                            return NoneValue.get();
    ...
    private Object executeComputationSync(ExecutionDAG.DAGVertex vertex) {
        ...
        if (parents.size() == 1) {
            ...
            System.out.println("\t* parent 0 iscanc=" + result.isCancelled() + "; isdone=" + result.isDone() + "; isdoneexc=" + result.isCompletedExceptionally());
