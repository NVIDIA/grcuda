...
void reset(float *auth1, float *auth2, float *hub1, float* hub2, float *auth_norm, float *hub_norm, int N, cudaStream_t s1, cudaStream_t s2) {
    for (int i = 0; i < N; i++) {
        auth1[i] = 1;
        auth2[i] = 1;
        hub1[i] = 1;
    ...
    // cudaMemPrefetchAsync(auth1, N * sizeof(int), 0, s2);
    // cudaMemPrefetchAsync(auth2, N * sizeof(int), 0, s1);
    // cudaMemPrefetchAsync(hub1, N * sizeof(int), 0, s1);
    // cudaMemPrefetchAsync(hub2, N * sizeof(int), 0, s2);
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < num_executions; i++) {
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            // spmv<<<nb, block_size, 0, s1>>>(ptr2, idx2, val2, hub1, auth2, N, nnz);
            spmv3<<<nb, block_size, block_size * sizeof(float), s1>>>(rowCounter1, ptr2, idx2, val2, hub1, auth2, N);
            err = cudaEventRecord(e1, s1);
            // spmv<<<nb, block_size, 0, s2>>>(ptr, idx, val, auth1, hub2, N, nnz);
            spmv3<<<nb, block_size, block_size * sizeof(float), s2>>>(rowCounter2, ptr, idx, val, auth1, hub2, N);
